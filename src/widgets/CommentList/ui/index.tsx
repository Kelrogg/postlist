...
export const CommentList: React.FC<CommentListProps> = ({ postId }) => {
  ...
  const cachedComms = useRef(new Map<number, IComment[]>())
  
  const filteredComms = useMemo(() => {
    if (cachedComms.current.has(postId)) {
      return cachedComms.current.get(postId) as IComment[];
    }

    const result = comments.filter(comment => comment.postId === postId);
    cachedComms.current.set(postId, result);
    return result;
  }, [postId]);
  ...
  return (
    <div className={styles['comment-list']}>
      <div className={styles['comment-list-header']}>
        <h3 className={styles['comment-list-title']}>Комментарии ({filteredComms.length})</h3>
      ...
      {isCollapsed || filteredComms.map(comment => (
        <CommentCard
          key={comment.id}
          comment={comment}
